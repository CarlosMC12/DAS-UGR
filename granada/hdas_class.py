"""
Created on Fri May 15 13:44:23 2020
...
class HDAS(object):
    def __init__(self, PathFileName: str, load: bool = True,
        ...
        self.RawMode = RawData
        if load:
            self.load(RawMode=self.RawMode)
    ...
    def __copy__(self):
        return HDAS(self.PathFileName, RawData=self.RawMode)
    ...
    def load(self, RawMode: bool = False):
        self.loadMatrix(RawMode=RawMode)
        self.set_time_n_fiber()
        self.set_time_window()

    def loadMatrix(self, RawMode: bool = False):
        """ load data set """
        try:
            [self.Data, self.Headers, self.N_processed_Points, self.N_Time_Samples] = LoadData(self.PathFileName, RawMode)
...
def LoadData(fullPath: str, RawData: bool = False) -> list:
    """
    ...
    return [TracesMatrix, Headers, N_processed_Points, N_Time_Samples]


if __name__ == '__main__':
    directory = r'C:\Users\abel\Documents\GITHUB\PYTHON\16-TracePacketGenerator\datos\2022_02_17_08h52m10s_HDAS_2Dmap_Strain.bin'
    # [TracesMatrix, Headers, N_processed_Points, N_Time_Samples] = LoadData(directory)
    hdas_prueba = HDAS(PathFileName=directory, load=True)
    foo = 1
